(function (exports) {
    ...
    function Monologue(opt) {
        ...
        const dict = { escape: true, backquote: true, sort_keys: false };
    ...
    Monologue.prototype.group = function group(grp, dir) {
        ...
        if(grp instanceof Array)		{ grp = grp.join(', '); }
    ...
    Monologue.prototype.order = function order(ord, dir) {
        ...
        if(ord instanceof Array)		{ ord = ord.join(', '); }
    ...
    Monologue.prototype.comparison = function comparison(p, sep, eq) {
        ...
        this.last_condition.call(this, p.map(function (val) {
            const str = this.stringify(val, eq).join(' AND ');
            return str;
        }.bind(this)), sep);
        } else if(Object(p) === p) {
    ...
    Monologue.prototype.query = function query() {
        ...
        if(this.parts.group.length > 0)		{ this.parts.query += ' GROUP BY ' + this.parts.group.join(','); }
    ...
    Monologue.prototype.stringify = function stringify(p, s, j) {
        ...
        for(let jj = 0, len = col.length; jj < len; ++jj) {
    ...
    Monologue.prototype.format = function format(v, k, s) {
        ...
        const vs = v.map(function (v) {
            return this.escape(v);
        }.bind(this)).join(',');

        const ret = (this.opt.backquote && k ? this.backquote(k) : k)
                + ' IN (' + vs + ')';
